...
ВАЖНО! введённую информацию НЕ хтмлсрепашчарю, поскольку это бэкенд, и админу может быть необходимо вводить html, например в поле ОПИСАНИЕ
Клиентский ввод ложился бы в базу через htmlspecialchars(). В этом случае в vue-шаблоне использовался бы атрибут v-html, а в партиале с доп полями - убрался бы htmlspecialchars при подставления значения в атрибут value.
